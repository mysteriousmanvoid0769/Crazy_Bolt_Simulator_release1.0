...
Для этого проверьте, что `qmake` добавлен в системный путь (`PATH`).
...
#### 3. Выполните команду для генерации Makefile:
```bash
qmake
```

#### 4. **Важно**: Если вы собираете проект во второй раз (или после изменений в исходных файлах), выполните команду `qmake clean`, чтобы очистить старые файлы сборки и обеспечить корректную пересборку:
```bash
make clean
```
...
#### 7. Перейдите в каталог с результатами сборки:
```bash
